/**
 * Gulp htmlcs reporter factory.
 * @module hny-gulp-htmlcs/reporter-factory
 */
...
export default function reporterFactory(config = {}, options = {}) {
     ...
     * @type {Function}
     */
    const formatter = typeof config.formatter === 'string' ? formatters[config.formatter] : config.formatter;
    ...
    return function reporter(results) {
        ...
        if (config.save) {
            asyncTasks.push(writer(formattedText, config.save, options.reportOutputDir));
        }
